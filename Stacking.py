...
model1 = Perceptron(max_iter=2000,tol=0.001,penalty='l2')
model2 = LogisticRegression(max_iter=3000,tol=0.001)
model3 = MLPClassifier(hidden_layer_sizes=(75,25,25),max_iter=2000)

stacking = StackingClassifier(
    estimators=[('model1', model1), ('model2', model2), ('model3', model3)],
    final_estimator=LogisticRegression(max_iter=3000,tol=0.001),
    cv=5
)
...
def dudoan():
    ...
    if ((Gender == '') or (Age == '') or (Income == '') or (Education == '') or (Marital_Status == '') or (Number_of_Children == '') or (Home_Ownership == '')):
        ...
        dudoan = stacking.predict([[int(Age),int(Gender), int(Income), int(Education), int(Marital_Status), int(Number_of_Children), int(Home_Ownership)]])
        if (dudoan == 0):
            str = "High"
        if (dudoan == 1):
            str = "Average"
